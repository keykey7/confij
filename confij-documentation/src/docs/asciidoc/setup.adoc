...
In case of dependency issues (like if an incompatible version of `com.fasterxml:classmate` is already on the classpath), an alternative shadowed artifact is available as `{group}:confij-core:{revnumberstable}:nodeps`.
