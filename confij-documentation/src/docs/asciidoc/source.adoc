...
The `AnySourceImpl` simply takes a URI-string and determines resource location and type based on it.
...
The `AnySourceImpl` expects a case-insensitive file ending of `.yaml` or `.yml`.
