...
Sources are where configuration values come from.
This can be anything from a file to git to envvars in various formats.
Confij will aways fail fast upon encountering an unknown value.

== Source Pipelines
ConfiJ loads sources one after the other.
Each consecutive source overrides (merges) attributes of the previous ones.
The last source therefore has the highest preference.
...
Source types can be freely mixed, meaning you can load from classpath,
then from a remote file, then from envionment variables and so on.
...
It is allowed to have incomplete configurations while there are
still more configuration sources to load.
However, it is never allowed to have an unknown configuration value.
Confij has a default fail-fast strategy such that you don't have to worry about legacy configs.
...
However only `@Default` values can be used for templating.

== The AnySource [[anysource]]

The `AnySource` simply takes a string and determines resource location and type based on it.
...
Most commonly the resource location (file, classpath,...)
is detected based on a URI scheme (see <<resource-locations>>).
The configuration format (yaml, json,...) is usually detected based on the ending.

== Source as Code

For more complicated scenarios where <<anysource>> is not suited, a `ConfijSource` must be passed explicitly.
Confij distinguishes between where something is read from (Resource) and what format it is.

.Explicitly written, but same sample as "<<pipedsource>>"
====
[source]
----
include::{src}[tag=pipedsource-explicit]
----
====
...
Register custom resource providers using a <<serviceloader>> for `ch.kk7.confij.source.any.ConfijAnyResource`.
...
The `AnySourceImpl` expects a case-insensitive file ending of `.hocon`, `.json` or `.conf`.
...
The `AnySourceImpl` expects a case-insensitive file ending of `.properties`.
