...
public class InterfaceBindingFactory implements ConfigBindingFactory<InterfaceBinding> {
	...
	@Override
	public Optional<InterfaceBinding> maybeCreate(ResolvedType type, ConfigBinder configBinder) {
		if (type.isInterface()) {
			...
			callStack.push(type);
			try {
				return Optional.of(new InterfaceBinding((ResolvedInterfaceType) type, configBinder));
			} finally {
				callStack.pop();
			}
		}
